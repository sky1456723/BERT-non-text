...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.args.random_shift:
            index_list = list(range(self.model.config.vocab_size))
            random.shuffle(index_list)
            index_list = torch.Tensor(index_list).unsqueeze(-1)
            self.random_map = nn.Embedding(num_embeddings = self.model.config.vocab_size, embedding_dim = 1)
            self.random_map.weight.data = index_list
            self.random_map = self.random_map.to(self.args.device)
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        err_msg = open(os.path.join(self.args.logging_dir, 'error_message.txt'), 'w')
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                try:
                    ...
                    err_msg.write(str(self.global_step))
                    err_msg.write("\n")
                    err_msg.write(str(e))
                    err_msg.write("\n")
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        ...
        for k, v in inputs.items():
            ...
            if k=='input_ids':                                                                                                                
                if self.args.random_shift:
                    print("[trainer.py] Doing random shift")
                    with torch.no_grad():
                        inputs[k] = self.random_map(inputs[k]).squeeze(-1).long()
                else:
                    inputs[k] = torch.remainder(inputs[k]+int(self.args.shift), self.model.config.vocab_size)
